...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func loadData() {
        ...
        // Deleting data already in database
        removeData()
        
        // define the json path
        guard let jsonURL = Bundle.main.url(forResource: "metros", withExtension: "json") else {
            print("Could not find metro.json!")
            return
        }
        // trying to read the json
        let jsonData = try! Data(contentsOf: jsonURL)
        
        // parsing the json into a swift object
        let stationsData = JSON(data: jsonData)
        
        // converting the datas into an array
